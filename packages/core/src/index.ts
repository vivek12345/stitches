import { tokenTypes, isServer, unitMatch, easingMatch } from './constants';
import {
  ATOM,
  IAtom,
  IBreakpoints,
    ...
    for (let index = 0; index < scaleTokenKeys.length; index++) {
      ...
      if (isMultiVarToken) {
        ...
        tokens[tokenType][token] = {
          // @ts-ignore
          original: tokens[tokenType][token],
        };
